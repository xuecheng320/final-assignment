...
public class SpriteSet {
    ...
    public void load(PApplet app,int CELL_W,int CELL_H,String walkFrameSet,String standFrameSet){
       ...
       //获取站立图
       walk = cutSheet(app,walkFrameSet);
       //获取站立图
       stand = cutSheet(app,standFrameSet);
       //获取攻击图
    }
    public void load(PApplet app,int CELL_W,int CELL_H,String standFrameSet){
       this.CELL_H = CELL_H;
       this.CELL_W= CELL_W;
       this.standFrameSet = standFrameSet;
       //获取站立图
    ...
    private PImage[][] cutSheet(PApplet app,String file){
        ...
        int rowUP = 2;
        int rowDOWN = 3;
        for(int i = 0;i<cols;i++){//(x,y,w,h)以x和y为左上角，切w*h大小的图片
